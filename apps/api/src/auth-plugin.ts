 ...
 */
const GATE_EXEMPT_PREFIXES = ['/health', '/auth/', '/billing/webhook', '/dev/'] as const;

function isGateExempt(url: string): boolean {
  const path = url.split('?', 1)[0]!;
  return GATE_EXEMPT_PREFIXES.some((p) =>
    p.endsWith('/') ? path.startsWith(p) || path === p.slice(0, -1) : path.startsWith(p),
  );
}

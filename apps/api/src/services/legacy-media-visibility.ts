...
export async function legacyMediaIsPubliclyReachable(
  ...
  const [row] = await client<{ known: boolean; live: boolean; erasing: boolean }[]>`
    ...
    select
      ...
      (exists(select 1 from erasing_media) or exists(select 1 from erasing_concepts)) as erasing
  `;
  // Unknown legacy files retain their historical behavior. Known files are
  // denied only at the exact all-erasing boundary; a foreign live association
  // keeps the shared byte reachable.
  return row ? !row.known || row.live || !row.erasing : false;
}

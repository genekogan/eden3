...
function isClosedReviewDatabase(raw: string, databaseName: string | null): boolean {
  if (databaseName === null || !/^eden3_runtime_e2e_review_[a-f0-9]{16}$/.test(databaseName) ||
      raw.includes('?') || raw.includes('#')) return false;
  let parsed: URL;
  try { parsed = new URL(raw); } catch { return false; }
  const port = Number(parsed.port);
  return (parsed.protocol === 'postgres:' || parsed.protocol === 'postgresql:') &&
    parsed.hostname === '127.0.0.1' && Number.isSafeInteger(port) && port >= 1024 && port <= 65_535 &&
    port !== 5432 && port !== 5433 && parsed.username === 'eden3_review_runtime' && parsed.password !== '' &&
    parsed.pathname === `/${databaseName}` && parsed.search === '' && parsed.hash === '';
}
...
export function runtimeAttestationFromEnvironment(
  ...
  const isClosedHarnessDatabase = databaseName !== null && (
    ...
    (/^eden3_runtime_e2e_(?!review_)[a-z0-9][a-z0-9_]{7,80}$/.test(databaseName) && isGate3Loopback) ||
    (/^eden3_voice_e2e_[a-f0-9]{16}$/.test(databaseName) && isLiteralIpv4) ||
    isClosedLoadHarnessDatabase(databaseUrl, databaseName) ||
    isClosedReviewDatabase(databaseUrl, databaseName)
  );
  if (
